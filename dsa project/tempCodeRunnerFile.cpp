...
void RailwayBookingSystem::displayTrains() {
    cout << left << setw(25) << "Train Name"
         ...
         << setw(15) << "First Class Price"
         << setw(15) << "Second Class Price"
         << setw(15) << "Third Class Price"
         << setw(15) << "Sleeper Price" << endl;

    cout << string(150, '-') << endl; // Separator line

    for (const auto& train : trains) {
        cout << left << setw(25) << train.TName
             << setw(15) << train.TID
             << setw(15) << train.TSource
             << setw(15) << train.TDest
             << setw(12) << train.TDate
             << setw(10) << train.TTime
             << setw(15) << train.TSeats
             << setw(15) << train.firstClassPrice
             << setw(15) << train.secondClassPrice
             << setw(15) << train.thirdClassPrice
             << setw(15) << train.sleeperPrice << endl;
    }
}
    ...
    for (const Train& train : trains) {
        if (train.TSource == from && train.TDest == to) {
            cout << "Train Name: " << train.TName 
                 ...
                 << ", First Class Price: " << train.firstClassPrice << endl;
        }
    }
...
void RailwayBookingSystem::loadBookedTicketsFromFile() {
    ...
    if (infile) {
        Ticket ticket;
        while (infile >> ticket.ticketID >> ticket.passengerName >> ticket.trainID >> ticket.seatNumber >> ticket.price) {
...
void RailwayBookingSystem::loadUsersFromFile() {
    ...
    if (infile) {
        User user;
        while (infile >> user.username >> user.password >> user.preferences) {
